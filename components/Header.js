        ...
        {links.map(
          link =>
            link.show && (
              <li
                key={link.id}
                className="block ml-4 text-black dark:text-gray-50 nav font-normal text-sm"
              >
...
export default function Header ({ navBarTitle, fullWidth }) {
  ...
  const resolveFavicon = fallback => !fallback && dark ? '/logo_text.svg' : '/favicon.png'
...
const HeaderName = forwardRef(function HeaderName ({ siteTitle, siteDescription, postTitle, onClick }, ref) {
  return (
    <p
      ref={ref}
      className="header-name ml-2 text-gray-600 dark:text-gray-300 capture-pointer-events grid-rows-1 grid-cols-1 items-center font-normal text-sm"
      onClick={onClick}
    >
